...
class AppDatabase extends _$AppDatabase {
  ...
  Future<List<ExpenseItem>> getExpenseItems(String expenseId) => (select(
    expenseItems,
  )..where((ei) => ei.expenseId.equals(expenseId))).get();

  Future<int> insertExpenseItem(ExpenseItemsCompanion item) =>
      into(expenseItems).insert(item);

  Future<int> deleteExpenseItems(String expenseId) => (delete(
    expenseItems,
  )..where((ei) => ei.expenseId.equals(expenseId))).go();
  ...
  Future<void> setKvValue(String key, String value) async {
    await into(kvStore).insertOnConflictUpdate(
      KvStoreCompanion(key: Value(key), valueJson: Value(value)),
    );
  }
